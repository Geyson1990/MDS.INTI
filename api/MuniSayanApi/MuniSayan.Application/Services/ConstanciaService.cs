...
using System.Linq;
using MuniSayan.Application.Mappers.Constancia;
using MuniSayan.Application.Mappers;
...
using MuniSayan.Application.Utils.Pdf;
    ...
    public class ConstanciaService : IConstanciaService
    {
        ...
        private async Task<Models.ValidationResult> ValidarEstudianteReniec(Models.Constancia.EstudiantePersonaRequest model)
        {
            ...
            try
            {
                ...
                DateTime nacimiento = DateTime.Parse(personaResult.fecNacimiento); //Fecha de nacimiento
                int edad = DateTime.Today.AddTicks(-nacimiento.Ticks).Year - 1;
                int mayoriaEdad = Int32.Parse(_configuration.GetSection("MayoriaEdad").Value);
                ...
                if (model.tipoSolicitante.Equals("1"))
                {
                    if (edad >= mayoriaEdad)
                    {
                        throw new ArgumentException("Los datos del estudiante corresponden a una persona de "+ mayoriaEdad.ToString() + " o más años. Ella o él deberá solicitar la constancia mediante la opción “A título personal”.");
                    }
                ...
                else if (model.tipoSolicitante.Equals("2")) 
                {
                    ...
                    } else if (edad >= mayoriaEdad && personaResult.fecFallecimientoSpecified)
                    {
                        throw new ArgumentException("El solicitante se encuentra fallecido según los registros del RENIEC. Si fuese un error, deberá comunicarse con el RENIEC.");
                    }
                }
        ...
        private async Task<Models.Constancia.EstudianteConstancia> SiagieEstudiante(string siagie, Models.Constancia.EstudianteModalidadNivelPersonaRequest request)
        {
            ...
            List<Models.Constancia.NotaModel> notasNorma2020 = await SiagieNotasEstudianteNorma2020(siagie, requestModalidadNivel);
            
            if (notasNorma2020 != null)
            {
                estudianteConstancia.notas.AddRange(notasNorma2020);
            }
        ...
        private async Task<List<Models.Constancia.NotaModel>> SiagieNotasEstudianteNorma2020(string siagie, Models.Constancia.EstudianteModalidadNivelRequest requestModalidadNivel)
        {
            try
            {
                var request = new
                ...
                var statusResponse = await _siagieService
                        .GetServiceByQueryAndToken<StatusResponse,
                        object>(siagie, "pdf/notas/norma2020", request);
        ...
        private byte[] PDFConstanciaInit(bool vistaPrevia, Models.Constancia.EstudianteConstancia estudianteConstancia)
        {
            try
            {
                ...
                string urlQRCode = _configuration.GetSection("PDF:QRCodeUrl").Value + "/validate/";
                string hashQRCode = urlQRCode + estudianteConstancia.solicitud.codigoVirtual;
                ...
                PdfProperties props = new PdfProperties(
                    ...
                    /*fontSize => */ 8,
                    /*sizeMargin => */ 18,
                    /*titulo => */ "CONSTANCIA DE LOGROS DE APRENDIZAJE",
                    /*urlQRCode => */ urlQRCode,
                    /*hashQRCode => */ hashQRCode,
                    /*estudiante => */ estudianteConstancia,
                    /*waterMarkText => */ _configuration.GetSection("PDF:WaterMarkTextConstancia").Value,
                    /*waterMarkFontSize => */ _configuration.GetSection("PDF:FontSize").Value,
                    /*anioLimiteV1 => */ 2019
                );
                Pdf constanciaPdf = new Pdf(props);

                return constanciaPdf.formatoConstancia().ToArray();
            }
            catch
            {
                return null;
