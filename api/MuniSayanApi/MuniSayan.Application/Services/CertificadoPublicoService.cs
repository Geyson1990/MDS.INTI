    ...
    public class CertificadoPublicoService : ICertificadoPublicoService
    {
                    ...
                    .Select(y => new Models.PDFObs
                    {
                        ...
                        obs = observaciones
                            ...
                            .Select(x => String.Format("{0} - {1} - {2}", x.idAnio.ToString(), x.resolucion.Trim().ToUpper(), x.motivo.Trim().ToUpper()))
                                       .Aggregate(new StringBuilder(), (current, next) => current.Append(next).Append(Environment.NewLine).Append(Environment.NewLine))
                                       .ToString()
                    })
                    .OrderBy(j => j.codTipo)
                    .ToList();
        ...
        private Models.CorreoModel PrepararCorreo(Models.Certificado.SolicitudCertificadoModel solicitud, string nombresApoderado, string correoElectronico)
        {
            string mensaje = "<html><body>Estimad@ <strong>" + nombresApoderado.ToUpper() + "</strong><br/><br/>" +
                   ...
                   "<strong>" + solicitud.fechaCreacion.ToString("HH:mm", CultureInfo.CreateSpecificCulture("es-PE")) + " hrs.</strong><br/><br/>." +
                   "De aprobarse la solicitud de Certificado Oficial de Estudios por el Directivo de la I.E., se le remitirá un correo con la confirmación " +
        ...
        private async Task<byte[]> PDFCertificadoDescarga(string codigoVirtual, string tipoDocumento, string numeroDocumento)
        {
            try
            {
                ...
                var resultSolicitud = await _unitOfWork.ObtenerSolicitudCertificadoPorCodigoVirtual2(codigoVirtual, tipoDocumento, numeroDocumento);
            ...
            List<Models.PDFObs> obsList)
        {
            ...
            using (Document document = new Document(pdf, PageSize.A4, false))
            {
                ...
                #region Header2
                table = new Table(UnitValue.CreatePercentArray(new float[] { 20, 60, 20 }));
                table.SetWidth(UnitValue.CreatePercentValue(100));
                table.SetHorizontalAlignment(HorizontalAlignment.CENTER);
                ...
                table = new Table(UnitValue.CreatePercentArray(new float[] { 30, 70 }));
                table.SetWidth(UnitValue.CreatePercentValue(100));
                table.SetHorizontalAlignment(HorizontalAlignment.CENTER);

                table.AddCell(PDFMinedu.getCell("Observaciones: ", 9, true, 1, 2, TextAlignment.LEFT, null, false));

                table.AddCell(PDFMinedu.getCell("Retiro(s): ", 9, false, 1, 1, TextAlignment.LEFT, null, false, VerticalAlignment.TOP));
                var obs = obsList.Where(x => x.codTipo == 1).Select(y => y.obs).FirstOrDefault();
                table.AddCell(PDFMinedu.getCell(obs == null ? "-" : obs.ToString(), 9, false, 1, 1, TextAlignment.LEFT, null, false));

                table.AddCell(PDFMinedu.getCell("Traslado(s): ", 9, false, 1, 1, TextAlignment.LEFT, null, false, VerticalAlignment.TOP));
                obs = obsList.Where(x => x.codTipo == 2).Select(y => y.obs).FirstOrDefault();
                table.AddCell(PDFMinedu.getCell(obs == null ? "-" : obs.ToString(), 9, false, 1, 1, TextAlignment.LEFT, null, false));

                table.AddCell(PDFMinedu.getCell("Prueba(s) de ubicación: ", 9, false, 1, 1, TextAlignment.LEFT, null, false, VerticalAlignment.TOP));
                obs = obsList.Where(x => x.codTipo == 3).Select(y => y.obs).FirstOrDefault();
                table.AddCell(PDFMinedu.getCell(obs == null ? "-" : obs.ToString(), 9, false, 1, 1, TextAlignment.LEFT, null, false));

                table.AddCell(PDFMinedu.getCell("Convalidación / Revalidación: ", 9, false, 1, 1, TextAlignment.LEFT, null, false, VerticalAlignment.TOP));
                ...
                tableFooter.AddCell(PDFMinedu.getCell(vistaPrevia ? " " : director, sizeFont, false, 1, 1, TextAlignment.CENTER, null, false));

                tableFooter.AddCell(PDFMinedu.getCell("* Este Certificado Oficial de Estudios puede ser verificado en el sitio web del " +
                ...
                "de poseer un software gratuito descargado de internet.\n" +
