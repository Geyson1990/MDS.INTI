...
namespace MuniSayan.Application.Services
{
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<StatusResponse> ObtenerRolPorModular(Models.Certificado.UsuarioModularRequest encryptedRequest)
        {
            var response = new StatusResponse();
            ...
            try
            {
                ...
                if (especialistaUgel == usuarioRequest.idRol)
                {
                    if(institucionEducativa.Estado == "1")
                    {
                        response.Success = false;
                        response.Data = null;
                        response.Messages.Add("Usted no tiene permisos para visualizar información de la IE. Se redireccionará a Siagie.");
                        return response;
                    }
                }
                else
                ...
                response.Data = new Models.Certificado.DatosUsuario()
                {
                    ...
                    //nombresUsuario = usuarioRequest.nombresUsuario,
                    nombresCompleto = usuarioRequest.nombreCompleto,
                    numeroDocumento = ReactEncryptationSecurity.Encrypt(usuarioRequest.numeroDocumento),
                    idRol = ReactEncryptationSecurity.Encrypt(usuarioRequest.idRol),
                    dscRol = menuNivel.dscRol.Trim(),
                    tipoSede = ReactEncryptationSecurity.Encrypt(usuarioRequest.tipoSede.ToString()),
                    ...
                    dscNivel = usuarioRequest.dscNivel.Trim(),
